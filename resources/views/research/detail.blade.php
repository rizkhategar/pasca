        ...
        <section class="profile-main-body">
            ...
            <section class="research-layout">
                <div id="scopus" class="tab-content active content-block research-card">
                    <div class="tab-header"><div><h2 class="block-title">Publikasi Internasional Scopus</h2><p>Daftar publikasi internasional terindeks Scopus.</p></div><div class="tab-badge"><i class="fas fa-database"></i>{{ $dosen->scopusPublications->count() ?? 0 }} Data</div></div>
                    <div class="table-responsive"><table class="table-data"><thead><tr><th class="col-no">No</th><th>Artikel & Info Jurnal</th><th class="col-year-q">Tahun / Q</th><th class="col-citation">Sitasi</th><th class="col-action">Aksi</th></tr></thead><tbody>@forelse($dosen->scopusPublications as $index => $scopus)<tr><td class="table-number">{{ $index + 1 }}</td><td><div class="pub-title">{{ $scopus->judul }}</div><div class="pub-muted">Jurnal: {{ $scopus->journal }}</div><div class="pub-muted">Penulis ke-{{ $scopus->author_order }} atau {{ $scopus->creator ?? '-' }}</div></td><td class="cell-center"><strong>{{ $scopus->tahun }}</strong>@if($scopus->quartile)<br><span class="badge-quartile badge-gap">{{ $scopus->quartile }}</span>@endif</td><td class="cell-center cell-strong cell-primary">{{ $scopus->citation }}</td><td class="cell-center">@if($scopus->url_artikel)<a href="{{ $scopus->url_artikel }}" target="_blank" class="pub-link"><i class="fas fa-external-link-alt"></i>Link</a>@else - @endif</td></tr>@empty<tr><td colspan="5" class="empty-text">Tidak ada data publikasi Scopus.</td></tr>@endforelse</tbody></table></div>
                </div>

                <div id="scholar" class="tab-content content-block research-card">
                    <div class="tab-header"><div><h2 class="block-title">Publikasi Google Scholar</h2><p>Daftar publikasi yang terindeks Google Scholar.</p></div><div class="tab-badge"><i class="fas fa-database"></i>{{ $dosen->scholarPublications->count() ?? 0 }} Data</div></div>
                    <div class="table-responsive"><table class="table-data"><thead><tr><th class="col-no">No</th><th>Judul Dokumen & Sumber</th><th class="col-year">Tahun</th><th class="col-citation">Sitasi</th><th class="col-action">Aksi</th></tr></thead><tbody>@forelse($dosen->scholarPublications as $index => $scholar)<tr><td class="table-number">{{ $index + 1 }}</td><td><div class="pub-title">{{ $scholar->judul }}</div><div class="pub-muted">Penulis: {{ $scholar->authors }}</div>@if($scholar->source)<div class="pub-muted">Sumber: {{ $scholar->source }}</div>@endif</td><td class="cell-center cell-strong">{{ $scholar->tahun }}</td><td class="cell-center cell-strong cell-primary">{{ $scholar->citation }}</td><td class="cell-center">@if($scholar->url_scholar)<a href="{{ $scholar->url_scholar }}" target="_blank" class="pub-link"><i class="fas fa-external-link-alt"></i>Link</a>@else - @endif</td></tr>@empty<tr><td colspan="5" class="empty-text">Tidak ada data publikasi Google Scholar.</td></tr>@endforelse</tbody></table></div>
                </div>

                <div id="garuda" class="tab-content content-block research-card">
                    <div class="tab-header"><div><h2 class="block-title">Publikasi Nasional Garuda</h2><p>Daftar publikasi nasional terindeks Garuda.</p></div><div class="tab-badge"><i class="fas fa-database"></i>{{ $dosen->garudaPublications->count() ?? 0 }} Data</div></div>
                    <div class="table-responsive"><table class="table-data"><thead><tr><th class="col-no">No</th><th>Judul Jurnal & Penerbit</th><th class="col-year">Tahun</th><th class="col-accreditation">Akreditasi</th><th class="col-action">Aksi</th></tr></thead><tbody>@forelse($dosen->garudaPublications as $index => $garuda)<tr><td class="table-number">{{ $index + 1 }}</td><td><div class="pub-title">{{ $garuda->judul }}</div><div class="pub-muted">Jurnal: {{ $garuda->journal }}</div><div class="pub-muted">Penerbit: {{ $garuda->publisher ?? '-' }}</div></td><td class="cell-center cell-strong">{{ $garuda->tahun }}</td><td class="cell-center"><span class="badge-soft">{{ $garuda->accreditation ?? '-' }}</span></td><td class="cell-center">@if($garuda->url_artikel)<a href="{{ $garuda->url_artikel }}" target="_blank" class="pub-link"><i class="fas fa-external-link-alt"></i>Link</a>@else - @endif</td></tr>@empty<tr><td colspan="5" class="empty-text">Tidak ada data publikasi Garuda.</td></tr>@endforelse</tbody></table></div>
                </div>

                <div id="research" class="tab-content content-block research-card">
                    <div class="tab-header"><div><h2 class="block-title">Histori Proyek Penelitian</h2><p>Riwayat penelitian dosen.</p></div><div class="tab-badge"><i class="fas fa-database"></i>{{ $dosen->researches->count() ?? 0 }} Data</div></div>
                    <div class="table-responsive"><table class="table-data"><thead><tr><th class="col-no">No</th><th>Judul Penelitian & Skema</th><th class="col-year">Tahun</th><th>Pendanaan</th><th class="col-status">Status</th></tr></thead><tbody>@forelse($dosen->researches as $index => $res)<tr><td class="table-number">{{ $index + 1 }}</td><td><div class="pub-title">{{ $res->judul }}</div><div class="pub-primary">Skema: {{ $res->skema ?? '-' }}</div><div class="pub-muted">Ketua: {{ $res->leader }} | Personil: {{ $res->personils ?? '-' }}</div></td><td class="cell-center cell-strong">{{ $res->tahun }}</td><td><span class="money-text">{{ $res->dana ?? '-' }}</span></td><td class="cell-center"><span class="badge-success">{{ $res->status ?? 'Selesai' }}</span></td></tr>@empty<tr><td colspan="5" class="empty-text">Tidak ada data histori penelitian.</td></tr>@endforelse</tbody></table></div>
                </div>

                <div id="service" class="tab-content content-block research-card">
                    <div class="tab-header"><div><h2 class="block-title">Histori Pengabdian Masyarakat</h2><p>Riwayat kegiatan pengabdian masyarakat.</p></div><div class="tab-badge"><i class="fas fa-database"></i>{{ $dosen->services->count() ?? 0 }} Data</div></div>
                    <div class="table-responsive"><table class="table-data"><thead><tr><th class="col-no">No</th><th>Judul Pengabdian & Kegiatan</th><th class="col-year">Tahun</th><th>Pendanaan</th><th class="col-status">Status</th></tr></thead><tbody>@forelse($dosen->services as $index => $serv)<tr><td class="table-number">{{ $index + 1 }}</td><td><div class="pub-title">{{ $serv->judul }}</div><div class="pub-primary">Skema: {{ $serv->skema ?? '-' }}</div><div class="pub-muted">Ketua: {{ $serv->leader }} | Personil: {{ $serv->personils ?? '-' }}</div></td><td class="cell-center cell-strong">{{ $serv->tahun }}</td><td><span class="money-text">{{ $serv->dana ?? '-' }}</span></td><td class="cell-center"><span class="badge-success">{{ $serv->status ?? 'Selesai' }}</span></td></tr>@empty<tr><td colspan="5" class="empty-text">Tidak ada data histori pengabdian masyarakat.</td></tr>@endforelse</tbody></table></div>
                </div>

                <div id="books" class="tab-content content-block research-card">
                    <div class="tab-header"><div><h2 class="block-title">Buku Karya Dosen Terdaftar</h2><p>Daftar buku karya dosen.</p></div><div class="tab-badge"><i class="fas fa-database"></i>{{ $dosen->books->count() ?? 0 }} Data</div></div>
