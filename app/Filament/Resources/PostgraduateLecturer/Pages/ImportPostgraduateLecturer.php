            ...
            <script>
                document.addEventListener('DOMContentLoaded', function () {
                    ...
                    const renderProdiSettings = (payload) => {
                        ...
                        prodiSummary.innerHTML = batch ? `Batch #${batch.id} | status: <b>${batch.status}</b> | ready: <b>${summary.ready_count || 0}</b> | belum setting: <b>${summary.missing_setting_count || 0}</b> | gagal: <b>${summary.failed_count || 0}</b> | belum fetch: <b>${summary.unfetched_count || 0}</b>` : 'Belum ada batch fetch. Jalankan Fetch All terlebih dahulu.';
                        ...
                        prodiModalBody.innerHTML = items.map((item) => {
                            ...
                            return `<tr><td style="padding:0.5rem;border-bottom:1px solid #e5e7eb;font-family:monospace;">${item.sinta_id}</td><td style="padding:0.5rem;border-bottom:1px solid #e5e7eb;">${item.lecturer_name || '-'}</td><td style="padding:0.5rem;border-bottom:1px solid #e5e7eb;color:${statusColor};font-weight:700;">${item.fetch_status}</td><td style="padding:0.5rem;border-bottom:1px solid #e5e7eb;"><select data-sinta-id="${item.sinta_id}" multiple ${disabled} style="width:100%;min-width:260px;border:1px solid #d1d5db;border-radius:0.375rem;padding:0.375rem;min-height:84px;">${options}</select></td><td style="padding:0.5rem;border-bottom:1px solid #e5e7eb;">${item.setting_status}</td></tr>`;
                        }).join('');
                    };
