...
class ImportDetailDosen extends Page implements HasSchemas
{
    ...
    protected static ?string $title = 'Import & Scraping SINTA Postgraduate';
        ...
        <div wire:ignore x-data="{
            init() {
                ...
                document.addEventListener('click', (event) => {
                    ...
                    resetTerminal('>>> Memulai migrasi streaming data Excel ke MySQL Postgraduate untuk SINTA ID: ' + sintaId + ' (study_program_id: ' + programStudiString + ')...' + NL);
                    toggleLoading(btnImport, true, 'Import ke Database');
                    let targetUrl = '{$urlImport}'.replace(':id', sintaId);
                    targetUrl += '?target=postgraduate&program_studi=' + encodeURIComponent(programStudiString);
                    openStream(targetUrl, () => {
    ...
    private function programStudyOptions(): array
    {
        $options = StudyProgram::query()
            ->where(function ($query): void {
                $query->where('jenjang', 'like', '%Magister%')
                    ->orWhere('jenjang', 'like', '%S2%')
                    ->orWhere('jenjang', 'like', '%Strata 2%');
            })
            ->orderBy('jenjang')
            ->orderBy('name')
            ->get()
        ...
        return ! empty($options) ? $options : StudyProgramOptions::postgraduateOptions();
    }
}
